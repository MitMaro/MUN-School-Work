...
public class Commands {
	...
	public int addReferenceCommand(String[] args) {
		...
		// this command didn't make any sense with my architecture so it does nothing
		return StatusCode.OK;
	}
	...
	public int sortCommand(String[] args) {
		...
		if (args.length == 2) {
			if (args[1].equals("PID")) {
				this.application.sortMasterList(SortOrder.PID, true);
			} else if (args[1].equals("author")) {
				this.application.sortMasterList(SortOrder.AUTHOR, true);
			}
		} else if (args.length == 4) {
			try {
				if (args[1].equals("PID")) {
					this.application.sortPaperList(SortOrder.PID, args[1], true);
				} else if (args[1].equals("author")) {
					this.application.sortPaperList(SortOrder.AUTHOR, args[1], true);
				}
			} catch (InvalidListException e) {
				this.terminal.out().println(e.getMessage());
				return StatusCode.MISSING_LIST;
	 ...
	 * 
	 * @param args The arguments passed to this command
	 * @return StatusCode.OK always
	 * @throws IllegalArgumentException is provided arguments are invalid
	 */
	@ca.mitmaro.commandline.command.annotation.Command(name="gui")
	@ca.mitmaro.commandline.command.annotation.Help(usage="gui", help="Start the graphical user interface")
	public int startGui(String[] args) {
	 ...
	 * @throws IllegalArgumentException is provided arguments are invalid
	 */
	@ca.mitmaro.commandline.command.annotation.Command(name="guionly")
	public int startGuiNoQuit(String[] args) {
