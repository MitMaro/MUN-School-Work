...
* the results in the form of a bar graph
* @param: none
* @returns: none
*
***********************************************************/
void lineCalibration() {
	...
	while (!button_is_pressed(BUTTON_B)) {// as long as B button is not pressed
	...
	while (button_is_pressed(BUTTON_B)) ;   // empty loop - wait until B button released
